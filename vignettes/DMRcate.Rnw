...
assays(tcell, withDimnames=FALSE)[["M"]] <- tcellms.noSNPs
assays(tcell, withDimnames=FALSE)[["Beta"]] <- ilogit2(tcellms.noSNPs)
@
...
Lastly, we would like to do a gene ontology test on our DMRs; this is made possible by the \texttt{goregion()} function in the \texttt{missMethyl} package. We will take the top 100 DMRs for this enrichment test.
