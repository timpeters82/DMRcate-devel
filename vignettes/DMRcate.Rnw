...
It is with this in mind that we filter out probes 2 nucleotides or closer to a SNP that have a minor allele frequency greater than 0.05, and the approximately 63,000 \cite{Pidsley, Chena} cross-reactive probes on either 450K and/or EPIC, so as to reduce confounding. Here we use a combination of \textit{in silico} analyses from \cite{Pidsley, Chena}.  About 600 are removed from our M-matrix of approximately 10,000:
...
DMRs are ranked by Fisher's multiple comparison statistic, but \texttt{Stouffer} scores and the harmonic mean of the individual component FDRs (\texttt{HMFDR}) are also given in this object. and pass it to DMR.plot, which uses the \texttt{Gviz} package as a backend for contextualising each DMR.  
...
The data contains 15 samples: 3 (unmatched) replicates of mouse Tregs from fat, liver, skin and lymph, plus a group of 3 CD4+ conventional T cells (Tcon). We will annotate the BSseq object to reflect this phenotypic information:
...
For demonstration purposes, we will retain CpGs on chromosome 2 only:

<<chr2filter>>=
bis_1072 <- bis_1072[seqnames(bis_1072)=="chr2",]
bis_1072
@
...
<<fitBSseq>>=
cont.mat <- makeContrasts(treg_vs_tcon=Lymph_N_Treg-Lymph_N_Tcon,
...
Say we want to find DMRs between the regulatory and conventional T cells from the lymph node. First we would fit the model, where \texttt{sequencing.annotate()} transforms counts into log2CPMs (via \texttt{limma::voom()}) and fits \texttt{limma} under the hood to generate per-CpG \textit{t}-statistics, indexing the FDR at 0.05: 
...
Looks like the top DMR is associated with the \textit{Tank} locus and hypomethylated in the Treg cells (since \texttt{meandiff < 0}). We can plot it like so:
...
Because this comparison is a bit more subtle, there are no significantly differential CpGs at this threshold. So we can use \texttt{changeFDR()} to relax the FDR to 0.25, taking into account that there is an increased risk of false positives.
...
All of the 31 DMRs found using results from \texttt{limma} are also found using \texttt{DSS::DMLtest.multiFactor()}, with an extra 27 DMRs found by the latter at the same FDR. This suggests that DMLtest.multiFactor() is a little more permissive in calling differential methylation.
...
\bibitem{Park}
  Park Y, and Wu H. Differential methylation analysis for BS-seq data under general experimental design. 
  \emph{Bioinformatics}. 2016 \textbf{32}(10), 1446-1453. 
