...
However, we can also select one probe from each replicate group based on its performance from cross-platform comparisons with EPICv1 and WGBS data (see Peters \textit{et al.} 2024)\cite{Peters2024}. Probe robustness is measured in terms of either: 
...
\bibitem{Peters2024}
  ...
  profiling. \emph{BMC Genomics} \bold{25}, 251. 
